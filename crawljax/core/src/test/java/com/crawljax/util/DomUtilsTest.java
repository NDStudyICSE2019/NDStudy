...
public class DomUtilsTest {
	...
	/*
	 * Tests the removal of hidden input tags.
	 * @throws IOException
	 */
	@Test
	public void testRemoveHiddenInputs() throws IOException {
		String html = "<body><input type=\"hidden\">" +
				"document.write(\"Testing!\")</input></body>";

		Document dom = DomUtils.asDocument(html);
		assertNotNull(dom);
		assertTrue(dom.getElementsByTagName("input").getLength() != 0);

		DomUtils.removeHiddenInputs(dom);
		assertTrue(dom.getElementsByTagName("input").getLength() == 0);
	}
