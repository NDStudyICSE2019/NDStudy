 ...
 * <li>A call to either <code>getChartImage()</code> or
 * <code>saveToFile(String)</code>.</li>
 * </ol>
 * 
 * <h3>Instantiation</h3>
 * <p>
...
public class HeatChart {
	...
	private Font axisValuesFont; // The font size will be considered the maximum font size - it may be smaller if
									// needed to fit in.
	...
	/**
	 * Returns the high value. This is the value at which the high value colour will
	 * be applied.
	 * 
	...
	/**
	 * Returns the 2-dimensional array of z-values currently in use. Each element is
	 * a double array which represents one row of the heat map, or all the z-values
	 * for one y-value.
	 * 
	 * @return an array of the z-values in current use, that is, those values which
	 *         will define the colour of each cell in the resultant heat map.
	 */
	 ...
	 * of y-values, with each element containing a double array with an equal number
	 * of elements that matches the number of x-values. Use this method where the
	 * minimum and maximum values possible are not contained within the dataset.
	 * 
	 * <h2>Example</h2>
	 * 
	 * <blockcode>
	 ...
	 * array must be the same length as the z-values array has columns. Each of the
	 * x-values elements will be displayed according to their toString
	 * representation.
	 * 
	 * @param xValues
	 ...
	 * array must be the same length as the z-values array has columns. Each of the
	 * y-values elements will be displayed according to their toString
	 * representation.
	 * 
	 * @param yValues
	...
	/**
	 * Returns the x-values which are currently set to display along the x-axis. The
	 * array that is returned is either that which was explicitly set with
	 * <code>setXValues(Object[])</code> or that was generated from the offset and
	 * interval that were given to <code>setXValues(double, double)</code>, in which
	 * case the object type of each element will be <code>Double</code>.
	 * 
	...
	/**
	 * Returns the y-values which are currently set to display along the y-axis. The
	 * array that is returned is either that which was explicitly set with
	 * <code>setYValues(Object[])</code> or that was generated from the offset and
	 * interval that were given to <code>setYValues(double, double)</code>, in which
	 * case the object type of each element will be <code>Double</code>.
	 * 
	 ...
	 *            true if x-values should be drawn horizontally, false if they
	 ...
	 *            true if y-values should be drawn horizontally, false if they
	 *            should be drawn vertically.
	 */
	...
	/**
	 * Sets the width of each individual cell that constitutes a value in x,y,z data
	 * space. By setting the cell width, any previously set chart width will be
	 * overwritten with a value calculated based upon this value and the number of
	 * cells in there are along the x-axis.
	 * 
	 * @param cellWidth
	 ...
	 * data space. By setting the cell height, any previously set chart height will
	...
	/**
	 * Sets the size of each individual cell that constitutes a value in x,y,z data
	 * space. By setting the cell size, any previously set chart size will be
	 * overwritten with a value calculated based upon this value and the number of
	...
	/**
	 * Returns the size of each individual data cell that constitutes a value in the
	 * x,y,z space.
	 * 
	...
	/**
	 * Returns the width of the chart in pixels as calculated according to the cell
	...
	/**
	 * Returns the height of the chart in pixels as calculated according to the cell
	 * dimensions, chart margin and other size settings.
	 * 
	...
	/**
	 * Returns the String that will be used as the title of any successive calls to
	 * generate a chart.
	 * 
	 ...
	 * generate a chart. The title will be displayed centralised horizontally at the
	 * top of any generated charts.
	 * 
	 * <p>
	...
	/**
	 * Sets the String that will be displayed as a description of the x-axis in any
	 * generated charts. The label will be displayed horizontally central of the
	 * x-axis bar.
	 * 
	 * <p>
	...
	/**
	 * Sets the String that will be displayed as a description of the y-axis in any
	 * generated charts. The label will be displayed horizontally central of the
	 * y-axis bar.
	 * 
	 * <p>
	...
	/**
	 * Returns an object that represents the colour to be used as the background for
	 * the whole chart.
	 * 
	 ...
	 * background can be set by setting a background colour with an alpha value. The
	 * transparency will only be effective when the image is saved as a png or gif.
	 * 
	 * <p>
	 ...
	 * 
	 * @param axisThickness
	 *            the thickness to use for the axis bars in any newly generated
	 *            charts.
	 */
	...
	/**
	 * Sets the font that describes the visual style of the axis labels. Both axis'
	...
	/**
	 * Returns the colour of the axis values as they will be painted along the axis
	 * bars. Both axis use the same colour.
	 * 
	...
	/**
	 * Sets the colour to be used for the axis values as they will be painted along
	 * the axis bars. Both axis use the same colour.
	 * 
	 * <p>
	 * Defaults to Color.BLACK.
	 * 
	...
	/**
	 * Returns the frequency of the values displayed along the x-axis. The frequency
	 * is how many columns in the x-dimension have their value displayed. A
	 * frequency of 2 would mean every other column has a value shown and a
	...
	/**
	 * Sets the frequency of the values displayed along the x-axis. The frequency is
	 * how many columns in the x-dimension have their value displayed. A frequency
	 * of 2 would mean every other column has a value and a frequency of 3 would
	 ...
	 *            the frequency of the values displayed against columns, where 1 is
	...
	/**
	 * Returns the frequency of the values displayed along the y-axis. The frequency
	...
	/**
	 * Sets the frequency of the values displayed along the y-axis. The frequency is
	 * how many rows in the y-dimension have their value displayed. A frequency of 2
	 * would mean every other row has a value and a frequency of 3 would mean every
	 * third row would be given a value.
	 * 
	 * <p>
	 * Defaults to 1. Every row is given a value.
	 * 
	 * @param axisValuesFrequency
	 *            the frequency of the values displayed against rows, where 1 is
	 *            every row and 2 is every other row.
	 */
	 ...
	 *            true if x-axis values should be displayed, false if they should be
	 *            hidden.
	 */
	 ...
	 *            true if y-axis values should be displayed, false if they should be
	 *            hidden.
	 */
	...
	/**
	 * Returns the scale that is currently in use to map z-value to colour. A value
	 * of 1.0 will give a <strong>linear</strong> scale, which will spread the
	 * distribution of colours evenly amoungst the full range of represented
	 * z-values. A value of greater than 1.0 will give an
	 * <strong>exponential</strong> scale that will produce greater emphasis for the
	 * separation between higher values and a value between 0.0 and 1.0 will provide
	 * a <strong>logarithmic</strong> scale, with greater separation of low values.
	 * 
	...
	/**
	 * Sets the scale that is currently in use to map z-value to colour. A value of
	 * 1.0 will give a <strong>linear</strong> scale, which will spread the
	 * distribution of colours evenly amoungst the full range of represented
	 * z-values. A value of greater than 1.0 will give an
	 * <strong>exponential</strong> scale that will produce greater emphasis for the
	 * separation between higher values and a value between 0.0 and 1.0 will provide
	 * a <strong>logarithmic</strong> scale, with greater separation of low values.
	 * Values of 0.0 or less are illegal.
	 * 
	 * <p>
	 ...
	 * other using an RGB coding with 0-255 values for each of red, green and blue.
	 * So the maximum colour distance is 255 + 255 + 255.
	 */
	 ...
	 * provided as a File parameter. The image type of the saved file will equal the
	 * extension of the filename provided, so it is essential that a suitable
	 * extension be included on the file name.
	 * 
	 * <p>
	 * All supported <code>ImageIO</code> file types are supported, including PNG,
	 * JPG and GIF.
	 * 
	 * <p>
	 ...
	 * 
	 * @param outputFile
	 *            the file location that the generated image file should be written
	 *            to. The File must have a suitable filename, with an extension of a
	 *            valid image format (as supported by <code>ImageIO</code>).
	 * @throws IOException
	 *             if the output file's filename has no extension or if there the
	 *             file is unable to written to. Reasons for this include a
	 *             non-existant file location (check with the File exists() method
	...
	/**
	 * Generates and returns a new chart <code>Image</code> configured according to
	 * this object's currently held settings. The given parameter determines whether
	 * transparency should be enabled for the generated image.
	 * 
	 * <p>
	 * No chart will be generated until this or the related
	 * <code>saveToFile(File)</code> method are called. All successive calls will
	 * result in the generation of a new chart image, no caching is used.
	 * 
	 * @param alpha
	...
	/**
	 * Generates and returns a new chart <code>Image</code> configured according to
	 * this object's currently held settings. By default the image is generated with
	 * no transparency.
	 * 
	 * <p>
	 * No chart will be generated until this or the related
	...
	/*
	 * Creates the actual heatmap element as an image, that can then be drawn onto a
	 * chart.
	 */
	...
	private void drawXLabel(Graphics2D chartGraphics) {
		if (xAxisLabel != null) {
			...
			// TODO This will need to be updated if the y-axis values/label can be moved to
			// the right.
	...
	/*
	 * Returns how many colour shifts are required from the lowValueColour to get to
	 * the correct colour position. The result will be different depending on the
	 * colour scale used: LINEAR, LOGARITHMIC, EXPONENTIAL.
	 */
