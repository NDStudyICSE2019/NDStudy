...
/**
 * A node of a tree. Each tree has an ID. The label can be empty, but can not
 * contain trailing spaces (nor consist only of spaces).
 * 
 * Two nodes are equal, if there labels are equal, and n1 < n2 if label(n1) <
 * label(n2).
 * 
 * @author Nikolaus Augsten from approxlib, available at
 *         http://www.inf.unibz.it/~augsten/src/ modified by Mateusz Pawlik
 */
public class LblTree extends DefaultMutableTreeNode implements Comparable {
	...
	/**
	 * tmpData: Hook for any data that a method must attach to a tree. Methods can
	 * assume, that this date is null and should return it to be null!
	 */
	...
	/**
	 * String representation of a tree. Reverse operation of
	 * {@link #fromString(String)}. treeID is NO_ID, it is skiped in the string
	 * representation.
	 *
