...
public class InfoTree {
	...
	public static final byte POST2_STRATEGY = 14; // strategy for Demaine (is there sth on the left/right of the heavy
													// node)
	...
	public boolean[][] nodeType; // store the type of a node: for every node stores three boolean values (L, R,
									// H)
	 ...
	 * stores in indexes. aInputTree is not needed any more. Remember to pass the
	 * same LabelDictionary object to both trees which are compared.
	 * 
	 * @param aInputTree
	 *            an LblTree object
	 * @param aLd
	...
	/**
	 * For given infoCode and postorder of a node returns requested information of
	 * that node.
	 * 
	 * @param infoCode
	 * @param nodesPostorder
	 *            postorder of a node
	...
	/**
	 * Returns relevant subtrees for given node. Assuming that child v of given node
	 * belongs to given path, all children of given node are returned but node v.
	 * 
	 * @param pathType
	 * @param nodePostorder
	 *            postorder of a node
	 ...
	 * current nodes children to assign them their parents.
	 * 
	 * @param aT
	 * @param postorder
	 * @return
	...
	private void postTraversalProcessing() {
		...
		for (i = 0; i < treeSize; i++) {
			...
			if (paths[RIGHT][i] == -1) {
				...
				info[RPOST2_RLD][treeSize - 1 - info[POST2_PRE][i]] = info[RPOST2_RLD][treeSize - 1
