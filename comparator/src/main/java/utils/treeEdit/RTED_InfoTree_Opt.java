...
public class RTED_InfoTree_Opt {
	...
	private static final byte POST2_STRATEGY = 14; // strategy for Demaine (is there sth on the left/right of the heavy
													// node)
	...
	private byte[][] deltaBit; // stores the distances difference of a form delta(F,G)-delta(F°,G°) for every
								// pair of subtrees, which is at most 1
	private double[][] t; // T array from Demaine's algorithm, stores delta(Fv,Gij), v on heavy path
		...
		for (int x = 0; x < sizes1.length; x++) { // for all nodes of initially left tree
			for (int y = 0; y < sizes2.length; y++) { // for all nodes of initially right tree
				...
				if (labels1[x] == labels2[y]) {
					...
					deltaBit[x][y] = 1; // if this set, the labels differ, cost of relabeling is set to costMatch
				}
	...
	private double spfL(InfoTree it1, InfoTree it2) {
		...
		return it1.isSwitched() ? delta[gPostorder][fPostorder] + deltaBit[gPostorder][fPostorder] * costMatch
		...
		for (int i1 = 1; i1 <= i - ioff; i1++) {
			for (int j1 = 1; j1 <= j - joff; j1++) {
				...
				if ((it1.info[POST2_LLD][i1 + ioff] == it1.info[POST2_LLD][i])
					...
					da = forestdist[i1 - 1][j1] + costDel;
					db = forestdist[i1][j1 - 1] + costIns;
					dc = forestdist[it1.info[POST2_LLD][i1 + ioff] - 1 - ioff][it2.info[POST2_LLD][j1 + joff] - 1
							- joff] + (switched ? delta[j1 + joff][i1 + ioff] : delta[i1 + ioff][j1 + joff]) + u;
					forestdist[i1][j1] = (da < db) ? ((da < dc) ? da : dc) : ((db < dc) ? db : dc);
				}
			}
		}
	...
	/**
	 * Single-path function for right-most path based on symmetric version of Zhang
	 * and Shasha algorithm.
	 * 
	 * @param it1
	 * @param it2
	 * @return distance between subtrees it1 and it2
		...
		for (int i1 = 1; i1 <= i - ioff; i1++) {
			for (int j1 = 1; j1 <= j - joff; j1++) {
				...
				if ((it1.info[RPOST2_RLD][i1 + ioff] == it1.info[RPOST2_RLD][i])
					...
					double u = 0;
					if (it1.info[POST2_LABEL][it1.info[RPOST2_POST][i1
							+ ioff]] != it2.info[POST2_LABEL][it2.info[RPOST2_POST][j1 + joff]]) {
						u = costMatch;
					}
					da = forestdist[i1 - 1][j1] + costDel;
					db = forestdist[i1][j1 - 1] + costIns;
					...
					setDeltaValue(it1.info[RPOST2_POST][i1 + ioff], it2.info[RPOST2_POST][j1 + joff],
					...
					da = forestdist[i1 - 1][j1] + costDel;
					db = forestdist[i1][j1 - 1] + costIns;
					dc = forestdist[it1.info[RPOST2_RLD][i1 + ioff] - 1 - ioff][it2.info[RPOST2_RLD][j1 + joff] - 1
							- joff]
							+ (switched ? delta[it2.info[RPOST2_POST][j1 + joff]][it1.info[RPOST2_POST][i1 + ioff]]
		...
		for (int i = gSize - 1; i >= 0; i--) {
			...
			for (int kPrime = 1; kPrime <= k; kPrime++) {
				...
				for (int j = jOfI; j >= 0; j--) {
					...
					if (kPrime == 1) {
						// if the direction changed from the previous period to this one use i and j of
						// previous strategy

						// since T is overwritten continously, thus use T_copy for getting values from
						// previous period
						...
						if (aStrategy != previousStrategy) {
							if (aStrategy == LEFT) {
								...
								previousI = gTreeSize - 1 - it2.info[RPOST2_POST][gTreeSize - 1 - gijForestPreorder]
				...
				if (i + jOfIminus1 < gSize) {
					...
					if (k - 1 - 1 < 0) {
						if (aStrategy != previousStrategy) {
							...
							setDeltaValue(it1.info[PRE2_POST][vpPreorder], it2.info[PRE2_POST][gijOfIMinus1Preorder],
	...
	/**
	 * Computes an array where preorder/rev.preorder of a subforest of given subtree
	 * is stored and can be accessed for given i and j.
	 * 
	 * @param it
	 ...
	 * @param treeSize
	 */
	private void computeIJTable(InfoTree it, int subtreePreorder, int subtreeRevPreorder, int subtreeSize,
			int aStrategy, int treeSize) {
	 ...
	 */
	private int jOfI(InfoTree it, int aI, int aSubtreeWeight, int aSubtreeRevPre, int aSubtreePre, int aStrategy,
			int treeSize) {
		return aStrategy == LEFT ? aSubtreeWeight - aI - it.info[POST2_SIZE][treeSize - 1 - (aSubtreeRevPre + aI)]
